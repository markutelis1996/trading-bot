...
REQUIRED: ALPACA_API_KEY, ALPACA_SECRET_KEY, CLICKUP_API_KEY, CLICKUP_WORKSPACE_ID, CLICKUP_CHANNEL_ID, TAVILY_API_KEY
OPTIONAL: PERPLEXITY_API_KEY (falls back to Tavily -> WebSearch if absent)
If a REQUIRED var is missing: STOP, send one ClickUp alert naming the missing var, exit.
MISSING OPTIONAL vars are OK — continue without them.
